...
<body>
<form name="ActionForm" method="post" action="${pageContext.request.contextPath}/crm/crmCustomerAction_updateShareSetOne.do" >
		...
		<c:if test="${! empty sysUserGroups }">
			...
			<fieldset style='padding:5px;clear:left;'>
					 ...
					 <c:if test="${! empty sysUsers }">
					 	...
					 	<c:forEach items="${sysUserList }" var="sysUserlist" >
					 		<c:if test="${sysUser.id==sysUserlist.id }">
					 			<c:set value="checked" var="xx"></c:set>
					 		</c:if>
					 	</c:forEach>
					 	<c:if test="${sysUser.sysUserGroup.id==sysUserGroup.id }">
                          <div>
						   <input type='checkbox' class='checkbox' name='uid' value='${sysUser.id }' id='uid10'  onclick='do_select(this.id)' ${xx }>${sysUser.cnName }
						  </div>&nbsp;
                        </c:if>  
                         </c:forEach>
                     </c:if>
