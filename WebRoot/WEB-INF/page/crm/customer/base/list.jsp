...
<script type="text/javascript">
 ...
 function deleteCompany(){
	...
	var chk = document.getElementById('ids');
	if(chk.checked){
 ...
 function do_share(){
	   ...
	   if(count==0){
		...
		if(count==1){  //选中一个客户的情况下
		   ...
		   OpenWin("${pageContext.request.contextPath}/crm/crmCustomerAction_showShareSetOne.do?id="+ids,'',500,400)
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabForm">
  ...
  <tr>
	  <td colspan="2">
<!-- 基本查询的表单-->
<s:form name="form1" method="post" action="crmCustomerAction_list.do" namespace="/crm">
<table width="100%" border="0" cellspacing="0" cellpadding="0" name="base" id="base">
	...
	<td>
		<s:if test="%{#request.gradeSelects!=null}">
			...
			name="grade" id="grade" listKey="value" listValue="value">
	...
	<td>
	 <s:if test="%{#request.sourceSelects!=null}">
	 	...
	 	headerKey="" headerValue="----"
	 	name="source" id="source" listKey="value" listValue="value">
...
<div class="control">
	<button type='button' class='button' onMouseOver="this.className='button_over';" onMouseOut="this.className='button';"  
	     onClick="forward('${pageContext.request.contextPath}/crm/crmCustomerAction_add.do')">
...
<div class="border">
<s:form method="post" action="crmCustomerAction_delete.do" namespace="/crm">
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="PowerTable" class="PowerTable">
	<tr>
    	<td width="3%" class="listViewThS1">
    	   <s:checkbox name="checkall" id="checkall" cssClass="checkbox" onclick="checkAll()"/>
    	</td>
	...
	<!-- data -->
		<s:if test="%{#request.companys != null}">
			<s:iterator value="%{#request.companys}" var="company">
			<tr>
		    	<td>
		    	   <s:checkbox id="ids" name="ids"  fieldValue="%{#company.id}" cssClass="checkbox" onclick="changeCheckCount();"/>
		    	</td>
		  	    <td>
		  	       <a href="${pageContext.request.contextPath}/crm/crmCustomerAction_edit.do?id=${company.id}"/><s:property value="%{#company.name}"/></a>
		  	    </td>
...
<script type="text/javascript">
	function goChangePerson() {
	  ...
	  var url = "${pageContext.request.contextPath}/crm/crmCustomerAction_showChangePerson.do?ids="+ids;
    ...
    function gonextTouchTime(){
	 ...
	 OpenWin("${pageContext.request.contextPath}/crm/crmCustomerAction_nextTouchTime.do?ids="+ids,'',420,300);
	}
