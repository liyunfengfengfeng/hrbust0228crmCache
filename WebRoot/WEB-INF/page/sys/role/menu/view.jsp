			 ...
			 <c:forEach items="${requestScope.sysMenus}" var="sysMenu">
			    <c:if test="${sysMenu.id.menuModule==sysMenu.id.menuPrivilege}">
			      <fieldset style='padding:5px;clear:left;'>
			            ...
			            <!-- 处理父菜单的选中状态 -->
			            <c:if test="${! empty requestScope.sysMenuPrivileges}">
				             <c:forEach items="${requestScope.sysMenuPrivileges}" var="sysMenuPrivilege">
						  ...
						  <!-- 处理父菜单的选中状态 -->
						   ...
						   <c:forEach items="${requestScope.sysMenus}" var="sysMenuSub">
						     <c:if test="${sysMenuSub.id.menuModule!=sysMenuSub.id.menuPrivilege
								...
								<!-- 处理子菜单的选中状态 -->
					            <c:if test="${! empty requestScope.sysMenuPrivileges}">
						             <c:forEach items="${requestScope.sysMenuPrivileges}" var="sysMenuPrivilegexx">
						         ...
						         <div><input type='checkbox' class='checkbox' name='menuModule'   ${xxx}
						               ...
						               id='${sysMenuSub.id.menuModule}_${sysMenuSub.id.menuPrivilege}'
						                    onClick='goSelect(this.id)' >${sysMenuSub.menuName}</div>
