	...
	<tr>
		...
		<s:if test="%{#request.sysUserGroups!=null}">
			<s:iterator value="%{#request.sysUserGroups}" var="sysUserGroup">
				<tr>
			    	<td>
					  <s:checkbox name="ids" fieldValue="%{#sysUserGroup.id}" cssClass="checkbox" onClick="changeCheckCount();"/>
					</td>
					...
					<td><s:property value="%{#sysUserGroup.principle}"/></td>
					<td><s:property value="%{#sysUserGroup.inclument}"/></td>
					<td><a href="javascript:OpenWin('${pageContext.request.contextPath}/sys/group/sysUserGroupAction_set.do?id=<s:property value="%{#sysUserGroup.id}" />')">设置</a></td>
				</tr>
			</s:iterator>
		</s:if>	
</table>
