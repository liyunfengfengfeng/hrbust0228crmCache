...
<script type="text/javascript">
	function checkTime(){
		...
		//处理毕业时间
		if(document.getElementById("finishSchoolTime").value==""){
			alert("毕业时间不能为空");
			return null;
		}
...
<div class="control">
	<button type='button' class='button' onMouseOver="this.className='button_over';" 
	        onMouseOut="this.className='button';"  onClick="checkTime()">
			...
			<tr>
				...
				<td>
					<s:if test="%{#request.sysRoles != null}">
						<s:select list="%{#request.sysRoles}" name = "roleId" id="roleId"
				...
				<td>
					<s:if test="%{#request.sysUserGroups != null}">
						<s:select list="%{#request.sysUserGroups}" name="groupId" id = "groupId"
							listKey="id" listValue="name">
  ...
  <tr>
  	<td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<td>职员类别：</td>
				<td><s:radio list="{'全职','兼职'}"  name="workerType" id="workerType" cssClass="radio"  value="%{'全职'}"/></td>
				<td>职务：</td>
