	...
	<tr>
  	    <td width="13%" nowrap="nowrap">用户名：</td>
   	    <td width="12%" nowrap="nowrap">
   	        <s:textfield name="name" id="name" cssStyle="width:100px" />
		</td>
  	    <td width="13%" nowrap="nowrap">中文名：</td>
		<td width="12%" nowrap="nowrap">
		    <s:textfield name="cnName" id="cnName" cssStyle="width:100px" />
		</td>
		<td width="13%" nowrap="nowrap">所属部门：</td>
		<td width="12%" nowrap="nowrap">
		   <!-- 获取部门数据在select标签中list迭代的集合listkey id listvalue 页面要显示的内容
		    headerKey不填 heardervalue页面刷出来时的默认显示 该标签要有默认的name 以及id
		            因为集合里是部门对象listkey 以及listvalue就可以直接去内容了-->
		   <s:if test="%{#request.sysUserGroups != null}">
		   		<s:select list="%{#request.sysUserGroups}" listKey="id" listValue="name" 
  	    ...
  	    <td width="25%" align="center">
			<div class="control">
				<button type='button' class='button' onMouseOver="this.className='button_over';" 
				        onMouseOut="this.className='button';"  onClick="searchUserInfo()">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="PowerTable" class="PowerTable">
  ...
  <s:if test="%{#request.sysUsers != null}">
	  <s:iterator value="%{#request.sysUsers}" var="sysUser">
		<tr>
	    	<td>
